...
# Write config/cache.env for compose env_file (container paths under /cache).
# Regenerated by bin/compose and bin/warm. Not machine-specific; gitignored by wire.
write_cache_env_file() {
  local dest="${1:-${ROOT}/config/cache.env}"
  local cache_root="${CACHE_ROOT:-/cache}"
  mkdir -p "$(dirname "${dest}")"
  cat >"${dest}" <<EOF
# Generated by cluster-tasks (bin/compose / bin/warm / bin/cache-env --write).
# Container paths for the shared Docker volume mounted at /cache.
# Compose:  env_file: [config/cache.env]  on app services (see x-app).
# Do not put secrets here. Host volume name is CACHE_VOLUME=${CACHE_VOLUME:-cache} (not this file).
#
CACHE_ROOT=${cache_root}
BUNDLE_PATH=${cache_root}/bundle
BUNDLE_CACHE_PATH=${cache_root}/rubygems
BUNDLE_USER_HOME=${cache_root}/bundler
BUNDLE_CLEAN=false
